...
export type FaderGradient = "warmth" | "hue" | "position" | "airflow" | "level";

/**
 * Fader vertical à rail creusé et curseur ivoire.
 *
 * Construit sur un input[type=range] pivoté : on hérite gratuitement du
 * tactile, du clavier et de la sémantique ARIA native, ce qu'une
 * réimplémentation au pointeur aurait perdu.
 *
 * Le maximum est en haut, le minimum en bas. Toute bande de couleur associée
 * doit donc placer sa valeur forte à 100 % d'un dégradé to top.
 */
@customElement("skeuo-fader")
export class SkeuoFader extends LitElement {
  @property({ type: Number }) public value = 0;
  @property({ type: Number }) public min = 0;
  @property({ type: Number }) public max = 100;
  @property({ type: Number }) public step = 1;
  @property({ type: String }) public caption = "";
  @property({ type: String }) public gradient: FaderGradient = "level";
  @property({ type: Boolean, reflect: true }) public disabled = false;
  /**
   * Le fader ne gouverne pas l'appareil en ce moment, mais reste manipulable :
   * s'en servir le remet aux commandes. À ne pas confondre avec `disabled`, qui
   * coupe l'interaction.
   */
